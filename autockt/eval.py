...
def rollout(agent, env_name, num_steps, out="assdf", no_render=True):
    """Roll out the agent for a number of steps.
    Args:
        agent (Agent): The RLlib agent to roll out.
        env_name (str): The name of the environment to use.
        num_steps (int): The number of steps to roll out.  (This is not
            necessarily the number of episodes, as some environments may
            have variable episode lengths. Currently, this is not used (args.steps))
        out (str): The output file to save the rollouts to.
        no_render (bool): If True, do not render the environment.
    Returns:
        None
    """
